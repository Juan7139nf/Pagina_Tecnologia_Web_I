...
if (isset($_GET['idu'])) {
    ...
    header("Location: /proyecto/mis/reserva/actualizar");
    exit;
}
...
if (isset($_GET['idd'])) {
    ...
    header("Location: /proyecto/reserva");
    exit;
}
?>
        ...
        <tbody>
            <?php foreach ($datos["reservas"] as $value): ?>
                <tr class="<?php 
                    ...
                    <td><?php echo $value['usuario_nombre'] ?> <?php echo $value['apellido_paterno'] ?> <?php echo $value['apellido_materno'] ?></td>
                    ...
                    <td class="p-0">
                        <?php if ($value['estado'] != 'confirmada'): ?>
                        <div class="btn-group">
                            <button class="btn btn-success" onclick="editarItem(<?php echo $value['id'] ?>)"><i class="bi bi-pencil-fill"></i></button>
                            <button class="btn btn-<?php echo ($value['estado'] == 'pendiente'? 'warning' : 'danger') ?>" onclick="eliminarItem(<?php echo $value['id'] ?>)">
                                <?php 
...
<script>
    function editarItem(id) {
        ...
        window.location.href = '/proyecto/reserva?idu=' + id;
    }

    function eliminarItem(id) {
        // Redirigir con el parámetro id en la URL
        window.location.href = '/proyecto/reserva?idd=' + id;
    }
</script>
